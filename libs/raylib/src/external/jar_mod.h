...
//  MODIFIED:
static void memcopy(void *dest, void *source, unsigned long size, unsigned long max) {
...
static bool jar_mod_load( jar_mod_context_t * modctx, void * mod_data, int mod_data_size )
{
    ...
    if(modmemory)
    {
        if( modctx )
        {
            ...
            // MODIFIED:
            memcopy(modctx->song.title, modmemory, 1084, sizeof(modctx->song.title));
            ...
            if( !modctx->number_of_channels )
            {
                ...
                // MODIFIED:
                memcopy(modctx->song.signature, "M.K.", 4, sizeof(modctx->song.signature));
                memcopy(&modctx->song.length, &(modctx->song.samples[15]), 1, sizeof(modctx->song.length));
    ...
    if( modctx && outbuffer )
    {
        if(modctx->mod_loaded)
        {
            ...
            if( trkbuf )
            {
                ...
                // MODIFIED:
                memcopy(trkbuf->name, modctx->song.title, sizeof(modctx->song.title), sizeof(trkbuf->name));
                ...
                for(i=0;i<31;i++)
                {
                    ...
                    // MODIFIED:
                    memcopy(trkbuf->instruments[i].name,modctx->song.samples[i].name, sizeof(modctx->song.samples[i].name), sizeof(trkbuf->instruments[i].name));
                }
            }
